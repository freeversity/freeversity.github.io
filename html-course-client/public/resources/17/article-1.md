...
Псевдокласс `:not(селектор)` является отрицающим селектором. С его помощью можно выбрать элементы, которые _НЕ_ содержат указанный селектор:
...
Псевдокласс `:last-of-type` выбирает _последний_ дочерний элемент родителя с учетом тега.

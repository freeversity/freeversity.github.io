...
- 
От типа поля зависит то, как оно будет отображаться и вести себя. Самый распространённый тип — это `text`, который обозначает _текстовое поле_. Данный тип используется по умолчанию.
...
Атрибут `id` поля ввода обозначает _идентификатор_. Он должен быть уникальным на всей странице.
...
Имена чекбоксов в рамках одной формы должны быть _разными_, так как чекбокс не подразумевает выбор одного элемента из нескольких.
